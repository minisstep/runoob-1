...
public class CasAuthenticationEntryPoint implements AuthenticationEntryPoint,
    ...
    public void commence(final HttpServletRequest request,
        ...
        if ("POST".equals(request.getMethod())||"XMLHttpRequest".equals(request.getHeader("X-Requested-With")) ||
                request.getHeader("Accept") != null &&  request.getHeader("Accept").contains("application/json")) {
            response.setHeader("Session-Status", "Session-Out");   //在响应头设置session状态
            response.setHeader("Redirect-Url", "www.baidu.com");//在响应头设置跳转URL
            response.setContentType("application/json;charset=UTF-8");
            PrintWriter out = response.getWriter();
            JSONObject json = new JSONObject();
            json.put("data", 302);
            json.put("class", "CasAuthenticationEntryPoint");
