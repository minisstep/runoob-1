...
/**
 * 在请求被路由之前检查HttpServletRequest中是否有accessToken参数，
 * 若有就进行路由，若没有就拒绝访问，返回401 Unauthorized错误。
 */
@Component
public class AccessZuulFilter extends ZuulFilter {
    private Logger log = LoggerFactory.getLogger(AccessZuulFilter.class);
    ...
    public Object run() {
        ...
        if (accessToken == null) {
            ...
            ctx.setResponseBody("access token is empty,example:http://www.example.com/service?accessToken=123");
            return null;
        }
